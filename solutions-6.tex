...
%TODO I think U_i should be X and V_j should be Y. Maps XxY -> U_i
    ...
    \item[1.\ (alternative)]{
        ...
        \item[2.\ (alternative)]{
            ...
            We now want to show that the given map is not injective in general, not even for
            affine schemes. To see this, define the following affine schemes
            \begin{align*}
                X=Y=\Spec(\mathbb{C}), S=\Spec(\mathbb{R}).
            \end{align*}
            Now $| \Spec(X\times_S Y)| \cong | \Spec(X\otimes_S Y)| \cong |\Spec(\mathbb{C}\times \mathbb{C})|=\lbrace(0,1),(1,0)\rbrace.$

            On the other hand, $\Spec(| X | \times_{| S |} | Y |)=\lbrace * \rbrace$ and clearly there is no injection from a
            set consisting of two points, to a set consisting of one point.
            }
    \end{enumerate}
\end{exercise}
