...
%TODO I think U_i should be X and V_j should be Y. Maps XxY -> U_i
    ...
    \item[1.\ (alternative)]{
        ...
        \item{
                We use the previous exercise. Let $X = \bigcup_i U_i = \Spec(A_i)$, $Y =
                \bigcup_i V_i = \Spec(B_i)$ and $S = \bigcup_{i, j} S_{i, j} =
                \Spec(R_{i, j})$ be coverings
                with affine schemes.

                By the previous exercise we get a covering $\bigcup_{i, j}
                \Spec(A_i \otimes_{R_{i, j}} B_j)$ with affine schemes.
                
                The maps $\Spec(A_i \otimes_{R_{i, j}} B_j) \to |\Spec(A_i)|
                \times_{|\Spec(R_{i, j})|} |\Spec(B_j)|$ are surjective for
                every $i, j$, and we have
                \begin{equation*}
